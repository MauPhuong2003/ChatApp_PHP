    ...
    public function register() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($username && $email && $phone && $password && $confirmPassword) {
                ...
                // Sử dụng email làm giá trị cho contact_email
                if ($this->userModel->register($username, $email, $phone, $passwordHash, $avatarPath)) {
                    $success_message = "Đăng ký thành công. Vui lòng đăng nhập!";
                    header("Location: index.php?action=login&msg=" . urlencode($success_message));
                    exit;
                } else {
                    $error = "Đăng ký thất bại. Email hoặc số điện thoại có thể đã được sử dụng.";
                    $this->render('register', ['error' => $error]);
                    return;
                }
            } else {
